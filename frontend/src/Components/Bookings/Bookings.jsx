...
import autoTable from 'jspdf-autotable'; // Correct import of autoTable
...
const Bookings = () => {
  ...
  // Function to download ticket as a PDF
  const downloadPDF = async (booking) => {
    ...
    try {
      ...
      // Generate barcode image using a temporary canvas
      ...
      // Base64 encoded logo placeholder (replace with your actual logo)
      ...
      doc.addImage(logoBase64, 'PNG', marginLeft, 15, 50, 50);
      doc.setFontSize(24);
      ...
      currentY += 20;
      doc.text(`Departure: ${booking.flight?.departureAirport || 'N/A'}`, marginLeft, currentY);
      currentY += 20;
      doc.text(`Arrival: ${booking.flight?.arrivalAirport || 'N/A'}`, marginLeft, currentY);
      currentY += 20;
      doc.text(
      ...
      // Passenger Details Table via autoTable
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      ...
      autoTable(doc, {
        ...
        body: booking.passengers.map((p, index) => [
          index + 1,
          p.name,
          p.age,
          p.gender,
