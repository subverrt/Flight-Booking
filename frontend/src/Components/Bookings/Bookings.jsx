...
const Bookings = () => {
  ...
  const downloadPDF = async (booking) => {
    ...
    try {
      ...
      // Generate barcode using a canvas element
      ...
      // Add logo (maintain aspect ratio)
      const logoWidth = 50; // Adjust as needed
      const logoHeight = 50; // Since the logo is square
      doc.addImage(logoBase64, 'PNG', marginLeft, 15, logoWidth, logoHeight);

      // Add company name "Fly High" next to the logo
      doc.setFontSize(24);
      doc.setTextColor(255, 255, 255);
      doc.setFont('helvetica', 'bold');
      doc.text('Fly High', marginLeft + logoWidth + 10, 50);
      ...
      currentY += 20;
      doc.text(`Airline: ${booking.flight?.airlineCode || 'N/A'}`, marginLeft, currentY);
      currentY += 20;
      doc.text(`Flight No: ${booking.flight?.flightNumber || 'N/A'}`, marginLeft, currentY);
      currentY += 20;
      doc.text(`Departure Airport: ${booking.flight?.departureAirport || 'N/A'}`, marginLeft, currentY);
      currentY += 20;
      doc.text(`Arrival Airport: ${booking.flight?.arrivalAirport || 'N/A'}`, marginLeft, currentY);
      currentY += 20;
      doc.text(
        `Departure Time: ${
      ...
      const qrXPosition = pageWidth - qrImageWidth - marginRight;
      const qrYPosition = marginTop + 20;
      doc.addImage(qrDataURL, 'PNG', qrXPosition, qrYPosition, qrImageWidth, qrImageHeight);
      ...
      // Add Barcode Image
      const barcodeImageWidth = 200;
      const barcodeImageHeight = 50;
      const barcodeXPosition = (pageWidth - barcodeImageWidth) / 2; // Centered horizontally
      const barcodeYPosition = currentY;
      doc.addImage(barcodeDataURL, 'PNG', barcodeXPosition, barcodeYPosition, barcodeImageWidth, barcodeImageHeight);
      ...
      doc.text(
        'Thank you for choosing Fly High! Have a pleasant journey.',
        pageWidth / 2,
        pageHeight - 30,
        { align: 'center' }
      );
      ...
      // Save the PDF
      doc.save(`FlyHigh_Ticket_${booking._id}.pdf`);
    } catch (error) {
