...
const Bookings = () => {
  ...
  // Generate a stunning PDF ticket that looks like a real airplane ticket.
const downloadPDF = (booking) => {
  if (!booking) return;

  // Create a new jsPDF instance (landscape, A5)
  const doc = new jsPDF({
  ...
  // Logo: Replace with your actual Base64 logo string.
  // If you do not have a valid logo yet, you can comment out the following block.
  ...
  try {
    ...
    console.warn("Logo image could not be added:", error);
  }

  // Title in header
  doc.setFontSize(22);
  doc.setTextColor(255, 255, 255);
  doc.text('Flight Ticket', pageWidth / 2, 35, { align: 'center' });
  ...
  // Left column: Booking & Flight Details
  doc.setFontSize(12);
  doc.setTextColor(0, 0, 0);
  let y = 90;
  const leftMargin = 20;
  doc.text(`Booking ID: ${booking._id}`, leftMargin, y);
  y += 20;
  doc.text(`Airline: ${booking.flight?.airline}`, leftMargin, y);
  y += 20;
  doc.text(`Flight No: ${booking.flight?.flightNumber}`, leftMargin, y);
  y += 20;
  doc.text(`From: ${booking.flight?.departureAirport}`, leftMargin, y);
  y += 20;
  doc.text(`To: ${booking.flight?.arrivalAirport}`, leftMargin, y);
  y += 20;
  doc.text(`Departure: ${new Date(booking.flight?.departureTime).toLocaleString()}`, leftMargin, y);
  y += 20;
  doc.text(`Seats: ${booking.seats ? booking.seats.join(', ') : 'None'}`, leftMargin, y);
  y += 20;
  doc.text(`Total: ₹${booking.totalAmount}`, leftMargin, y);

  // Right column: Passenger Details
  let rightY = 90;
  const midX = pageWidth / 2 + 10;
  doc.text('Passenger Details:', midX, rightY);
  rightY += 20;
  booking.passengers.forEach((passenger, index) => {
    doc.text(`${index + 1}. ${passenger.name} (${passenger.age} yrs, ${passenger.gender})`, midX, rightY);
    rightY += 20;
  });
  ...
  doc.text('Thank you for choosing our airline!', pageWidth / 2, pageHeight - 20, { align: 'center' });
